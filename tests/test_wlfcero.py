...
def getEdges(poly):
  # 01. GET POINT PLACE AND BBOX
    ...
    x = lon_point_list
    y = lat_point_list
    point = lat1,lng1 =sum(x)/len(x), sum(y)/len(y) 
    ...
    for i in multi_line:
        x,y = i.coords.xy
        starti_points.append(pit(x[0],y[0]))
        endi_points.append(pit(x[-1],y[-1]))
    
    middle_points = wlfc.mid_pts(starti_points,endi_points)
    print(middle_points)
    ...
    # gdf_proj_streets = ox.projection.project_gdf(gdf_proj_streets, to_crs=4326)
    # return gdf_proj_streets, point
    return prueba
